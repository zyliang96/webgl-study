...
export default function index(props) {
    ...
    const vertexShader = `
        ...
        // 角度转弧度
        float angle = radians(u_Translation);
        float sinB = sin(angle);
        float cosB = cos(angle);
        // attribute float a_PointSize; 
        void main() {
            gl_Position.x = a_Position.x*cosB-a_Position.y*sinB; // 设置坐标 值都是0-1区间内 字段分别为坐标 x,y,z 
            gl_Position.y = a_Position.y*cosB+a_Position.x*sinB; // 设置坐标 值都是0-1区间内 字段分别为坐标 x,y,z 
            gl_Position.z = a_Position.z; // 设置坐标 值都是0-1区间内 字段分别为坐标 x,y,z 
